...
interface ISurface extends Component<SurfaceProps, *> {
  ...
  +capture:(location?: String)=>[String,number, number];
...
export default ({
  ...
  return class Surface extends Component<*, *> {
     ...
     * @instance
     */
    async capture(location?: string): [string,number, number] {
      // invariant(
      //   this.root,
      //   "Surface#capture: surface is not yet ready or don't have any root Node"
      // );
      // return this.root.capture(x, y, w, h);
      const { glView,gl } = this;
        return await  glView.capture(gl,location);
    }
